...
module.exports = class HangmanCommand extends Command {
  ...
  run(msg, { guess: action }) {try {
    ...
    if (game.guessedLetters.includes(action)) return msg.channel.send(this.client.utils.embed().setColor("RED")
      .setAuthor(msg.member.displayName, msg.author.displayAvatarURL).setTitle("Hangman game error").setDescription("You've already guessed that letter"))
    game.guess(action)
    
    if (game.status != "IN_PROGRESS") {
      this.client.points.ensure(key, this.client.defaultPoints(msg));
