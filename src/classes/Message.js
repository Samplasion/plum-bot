...
module.exports = Structures.extend("Message", Message => class PlumMessage extends Message {
    ...
    makeEmbed(author = false) {
        let e = this.client.utils.embed();
        if (author) {
            e.setAuthor(this.member.displayName, this.author.displayAvatarURL({ type: "png", dynamic: true }))
        }
        if (this.noEmbed) {
            e.textOnly = true;
        }
    ...
    t(str, opts = {}) {
        return i18n.translate(this.client, this.guild.id || "", str, { ...opts });
    }
});
